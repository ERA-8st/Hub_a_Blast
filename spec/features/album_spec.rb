  ...
  describe "アルバム詳細のテスト" do
    ...
    context "コメントのテスト" do
      ...
      context "投稿のテスト" do
        ...
        it "フォームが表示されない" do
          expect(album_show_comments).to have_selector "textarea#song_comment_comment"
          expect(album_show_comments).to haveselector "input[value='送信する']"
          click_link "LOG_OUT"
          visit user_spotify_album_show_path("0qsh1whWV3FcVaM6A8vLN9")
          expect(album_show_comments).to_not have_selector "textarea#song_comment_comment"
          expect(album_show_comments).to_not have_selector "input[value='送信する']"
        end
      end
      context "更新のテスト" do
        before do
        ...
        it "失敗する" do
          fill_in "alubm_comment#{album_comment.id}_edit_form", with: ""
          click_button "更新する"
          expect(album_show_comments).to have_content "コメントを入力してください"
        end
        it "取り消せる" do
        ...
        it "表示されない" do
          # visit user_spotify_album_show_path("0qsh1whWV3FcVaM6A8vLN9")
          expect(album_comment).to_not have_link "album_comment#{album_comment2.id}_edit"
        end
      end
      context "削除のテスト" do
        it "成功する" do
